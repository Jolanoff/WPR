...
const SchademeldingenPage = () => {
  ...
  const handleStatusChange = async (id, newStatus) => {
    try {
      const response = await api.post(`/Schademeldingen/switch_status/${id}?newStatus=${newStatus}`);
      const updatedSchade = response.data;

      setSchademeldingData((prevData) =>
        prevData.map((item) => (item.id === id ? { ...item, status: updatedSchade.status } : item))
      );
          ...
          <table className="min-w-full bg-white border border-gray-200">
            ...
            <tbody>
              {schademeldingData.map((item, index) => (
                <tr key={index}>
                  <td className="py-2 px-4 border-b">{new Date(item.datum).toLocaleDateString()}</td>
                  ...
                  <td className="py-2 px-4 border-b">
                    <div className="flex gap-2">
                      <button
                        onClick={() => handleStatusChange(item.id, "InAfwachting")}
                        className="bg-yellow-400 text-white px-2 py-1 rounded hover:bg-yellow-500"
                      >
                        InAfwachting
                      </button>
                      <button
                        onClick={() => handleStatusChange(item.id, "Goedgekeurd")}
